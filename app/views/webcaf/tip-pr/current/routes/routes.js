...
function getOtherTaskHref() {
  return `${BASE}/task/2/other-recommendations-summary`
}
...
function getOtherRecommendationHref(rec) {
  ...
  return `${BASE}/task/2/${rec.recId.toLowerCase()}/review`
}
...
router.get('/draft-tip', function (req, res) {
  ...
  res.render('webcaf/tip-pr/current/draft-tip', {
    ...
    priorityTaskHref: `${BASE}/task/1/priority-c`,
...
router.get('/task/1/:recSlug/review', function (req, res) {
  const tip = buildTipState(req.session.data)
...
router.post('/task/1/:recSlug/review', function (req, res) {
  const tip = buildTipState(req.session.data)
  const recId = req.params.recSlug.toUpperCase()
  ...
  if (nextRecId) {
    return res.redirect(`${BASE}/task/1/${nextRecId.toLowerCase()}/review`)
  }
...
router.get('/task/2/:recSlug/review', function (req, res) {
  const tip = buildTipState(req.session.data)
  const recId = req.params.recSlug.toUpperCase()
...
router.post('/task/2/:recSlug/review', function (req, res) {
  const tip = buildTipState(req.session.data)
  const recId = req.params.recSlug.toUpperCase()
...
router.post('/task/2/review-all', function (req, res) {
  const tip = buildTipState(req.session.data)
  ...
  return res.redirect(`${BASE}/task/2/other-recommendations-summary`)
})
